...
3. `content.json` in this repository must stay byte-identical to the pipeline file; the deploy script overwrites it and asserts only on ids. Edit questions in the pipeline.
   **Superseded the same day.** The case-studies merge (`docs/prose-audit-2026-09-02.md`) rewrote the eight `caseStudy` strings here, so the file now differs from the pipeline's in exactly those eight lines; the 439 questions are still byte-identical. `securepathdigital-site/stage-learn.sh` copies the pipeline file over this one and asserts only on ids, so as written it would silently discard the new case studies. Either give the pipeline file the same eight strings (and add `What a practitioner does differently` to the builder's `DEBRIEF_RE`, as `src/lib/text.ts` now has, or the book prints the analysis before the questions), or drop that copy step, which the script says can go once the rekeyed bank is committed here. Questions still belong in the pipeline.
   **2026-09-03.** The eight `caseStudy` strings are the printed narrative again (title line, intro, Key Events, Analysis, Lessons), each with the rewrite's "Related reading" links kept as a tail, plus five wording fixes from `audit-prose` (two ", ensuring" clauses, "highlights", "Additionally," with its "where kept" typo, one filler "comprehensive"). The 439 questions are still byte-identical to the pipeline file. `stage-learn.sh` no longer copies over `content.json` (branch `journal-2026-09-03/stage-learn-fix`). On flag 2: the app's `DEBRIEF_RE` now also matches `**Analysis:**` and "The Failure Analysis:", so the app holds all three heading forms back; the builder still prints two of them before the questions.
